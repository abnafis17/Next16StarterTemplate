...
interface TanstackDataTableProps<TData> extends React.HTMLAttributes<HTMLDivElement> {
  data: any;
      ...
      <Table>
        <TableHeader className="">
          {table.getHeaderGroups().map((headerGroup) => (
            <TableRow key={headerGroup.id} className={isBorderless ? 'border-none' : ''}>
          ...
          {table.getRowModel().rows?.length > 0 ? (
            ...
            <TableRow className={isBorderless ? 'border-none h-full' : ''}>
              <TableCell
                ...
                className={`h-24 text-center ${isBorderless ? 'border-none' : ''}`}
              >
                No Data Available
              </TableCell>
            </TableRow>
